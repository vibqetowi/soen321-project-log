...
public class ShareInviteHelper {
    public static LinkGenerator generateInviteUrl(Context context) {
        ...
        LinkGenerator addParameter = new LinkGenerator("af_app_invites").setBaseURL(string, string2, context.getPackageName()).setReferrerUID(appsFlyerUID).setReferrerCustomerId(ac.AFInAppEventType()).addParameter("af_siteid", context.getPackageName());
    ...
    public static void logInvite(Context context, String str, Map<String, String> map) {
        if (TextUtils.isEmpty(str)) {
            ...
            AFLogger.AFInAppEventParameterName("[Invite] Reporting App-Invite via channel: ".concat(String.valueOf(str)));
