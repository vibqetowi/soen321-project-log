...
public class Debug {
    ...
    public static void printStack(String msg, int n) {
        ...
        for (int i = 1; i <= min; i++) {
            ...
            printStream.println(msg + str + (".(" + stackTrace[i].getFileName() + CertificateUtil.DELIMITER + stackTrace[i].getLineNumber() + ") ") + str);
        }
    }
    ...
    public static String getName(Context context, int[] id) {
        ...
        try {
            ...
            Log.v(OkHttpClientProviderKt.DEBUG_KEY, e.toString());
            return "UNKNOWN";
        }
    }
    ...
    public static String getState(MotionLayout layout, int stateId, int len) {
        ...
        if (len != -1) {
            ...
            return resourceEntryName.replaceAll(CharBuffer.allocate(length2).toString().replace((char) 0, ClassUtils.PACKAGE_SEPARATOR_CHAR) + "_", "_");
        }
        return resourceEntryName;
    }
