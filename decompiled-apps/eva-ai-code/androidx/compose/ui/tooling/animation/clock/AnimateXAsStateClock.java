...
public final class AnimateXAsStateClock<T, V extends AnimationVector> implements ComposeAnimationClock<AnimateXAsStateComposeAnimation<T, V>, TargetState<T>> {
    ...
    public AnimateXAsStateClock(AnimateXAsStateComposeAnimation<T, V> animation) {
        ...
        this.state = new TargetState<>(getAnimation().m5523getAnimationObject().getValue(), getAnimation().m5523getAnimationObject().getValue());
    ...
    public List<TransitionInfo> getTransitions(long j) {
        return CollectionsKt.listOf(UtilsKt.createTransitionInfo(this.currAnimation, getAnimation().getLabel(), getAnimation().getAnimationSpec(), j));
    }
    ...
    private final TargetBasedAnimation<T, V> getCurrentAnimation() {
        return AnimationKt.TargetBasedAnimation(getAnimation().getAnimationSpec(), getAnimation().m5523getAnimationObject().getTypeConverter(), getState().getInitial(), getState().getTarget(), getAnimation().m5523getAnimationObject().getVelocity());
    }
}
