...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m4893toOffsetUv8p0NA(int[] iArr, long j) {
        float coerceAtLeast;
        float coerceAtLeast2;
        if (Offset.m3018getXimpl(j) >= 0.0f) {
            coerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m3018getXimpl(j));
        } else {
            coerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m3018getXimpl(j));
        }
        if (Offset.m3019getYimpl(j) >= 0.0f) {
            coerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m3019getYimpl(j));
        } else {
            coerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m3019getYimpl(j));
        }
    ...
    public static final int m4894toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m4317equalsimpl0(i, NestedScrollSource.Companion.m4322getDragWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: getScrollAxes-k-4lQ0M  reason: not valid java name */
    public static final int m4892getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m3018getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m3019getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
