...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString text, long j) {
        ...
        this.selectionStart = TextRange.m5080getMinimpl(j);
        this.selectionEnd = TextRange.m5079getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m5080getMinimpl = TextRange.m5080getMinimpl(j);
        int m5079getMaximpl = TextRange.m5079getMaximpl(j);
        if (m5080getMinimpl < 0 || m5080getMinimpl > text.length()) {
            throw new IndexOutOfBoundsException("start (" + m5080getMinimpl + ") offset is outside of text region " + text.length());
        } else if (m5079getMaximpl < 0 || m5079getMaximpl > text.length()) {
            throw new IndexOutOfBoundsException("end (" + m5079getMaximpl + ") offset is outside of text region " + text.length());
        } else if (m5080getMinimpl <= m5079getMaximpl) {
        } else {
            throw new IllegalArgumentException("Do not set reversed range: " + m5080getMinimpl + " > " + m5079getMaximpl);
        }
    }
    ...
    public final TextRange m5247getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m5070boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release  reason: not valid java name */
    public final long m5248getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int i, int i2) {
        ...
        long m5249updateRangeAfterDeletepWDy79M = EditingBufferKt.m5249updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m5080getMinimpl(m5249updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m5079getMaximpl(m5249updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m5249updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m5249updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m5076getCollapsedimpl(m5249updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
                return;
            }
            this.compositionStart = TextRange.m5080getMinimpl(m5249updateRangeAfterDeletepWDy79M2);
            this.compositionEnd = TextRange.m5079getMaximpl(m5249updateRangeAfterDeletepWDy79M2);
        }
    }
