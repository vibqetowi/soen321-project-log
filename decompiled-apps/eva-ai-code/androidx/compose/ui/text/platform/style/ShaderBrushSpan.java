...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float f) {
        ...
        this.alpha = f;
        this.size = Size.Companion.m3095getUnspecifiedNHjbRc();
    }

    /* renamed from: getSize-NH-jbRc  reason: not valid java name */
    public final long m5360getSizeNHjbRc() {
        return this.size;
    }

    /* renamed from: setSize-uvyYCjk  reason: not valid java name */
    public final void m5361setSizeuvyYCjk(long j) {
        this.size = j;
    }

    @Override // android.text.style.CharacterStyle
    public void updateDrawState(TextPaint textPaint) {
        Shader mo3260createShaderuvyYCjk;
        Intrinsics.checkNotNullParameter(textPaint, "textPaint");
        AndroidTextPaint_androidKt.setAlpha(textPaint, this.alpha);
        if (this.size == Size.Companion.m3095getUnspecifiedNHjbRc()) {
            return;
        }
        Pair<Size, ? extends Shader> pair = this.cachedShader;
        if (pair == null || !Size.m3083equalsimpl0(pair.getFirst().m3092unboximpl(), this.size)) {
            mo3260createShaderuvyYCjk = this.shaderBrush.mo3260createShaderuvyYCjk(this.size);
        } else {
            mo3260createShaderuvyYCjk = pair.getSecond();
        }
        textPaint.setShader(mo3260createShaderuvyYCjk);
        this.cachedShader = TuplesKt.to(Size.m3075boximpl(this.size), mo3260createShaderuvyYCjk);
    }
}
