...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (textLayoutResult.getDidOverflowHeight() && !textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                lineCount = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m5766getHeightimpl(textLayoutResult.m5055getSizeYbymL2g())), textLayoutResult.getLineCount() - 1);
                while (textLayoutResult.getLineTop(lineCount) >= IntSize.m5766getHeightimpl(textLayoutResult.m5055getSizeYbymL2g())) {
                    lineCount--;
                }
    ...
    public Pair<Selection, Boolean> mo1153updateSelectionqCDeeow(long j, long j2, Offset offset, boolean z, LayoutCoordinates containerLayoutCoordinates, SelectionAdjustment adjustment, Selection selection) {
        ...
        if (layoutCoordinates != null && (invoke = this.layoutResultCallback.invoke()) != null) {
            long mo4553localPositionOfR5De75A = containerLayoutCoordinates.mo4553localPositionOfR5De75A(layoutCoordinates, Offset.Companion.m3034getZeroF1C5BW0());
            return MultiWidgetSelectionDelegateKt.m1157getTextSelectionInfoyM0VcXU(invoke, Offset.m3022minusMKHz9U(j, mo4553localPositionOfR5De75A), Offset.m3022minusMKHz9U(j2, mo4553localPositionOfR5De75A), offset != null ? Offset.m3007boximpl(Offset.m3022minusMKHz9U(offset.m3028unboximpl(), mo4553localPositionOfR5De75A)) : null, getSelectableId(), adjustment, selection, z);
        }
    ...
    public Selection getSelectAllSelection() {
        ...
        return MultiWidgetSelectionDelegateKt.m1154access$getAssembledSelectionInfovJH6DeI(TextRangeKt.TextRange(0, invoke.getLayoutInput().getText().length()), false, getSelectableId(), invoke);
    }

    @Override // androidx.compose.foundation.text.selection.Selectable
    /* renamed from: getHandlePosition-dBAh8RU  reason: not valid java name */
    public long mo1151getHandlePositiondBAh8RU(Selection selection, boolean z) {
        TextLayoutResult invoke;
    ...
    public long mo1152getRangeOfLineContainingjx7JFs(int i) {
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
