...
- MobSF: First, it kept crashing, so this may be not entirely accurate, but it is what it is.
  ...
  - It is also revealed that all the main APIs go to India. The ones going to the EU and US are either stuff like Google, Facebook, Apple, or standard analytics like the 7 trackers identified. This could be a concern considering that I am using an USA vpn therefore am technically an USA customer and mental health data is under HIPAA. Long story short, I am not certain sending api calls to Indian servers is entirely legal for this app. This is further compounded by the fact that when the app sends data to the Indian API and identifies users by `uid = email` and sends that unencrypted.
